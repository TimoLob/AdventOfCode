...
fn parse(input:&str) -> IResult<&str,Vec<Race>> {
    ...
    let (_,times) = preceded(tuple((tag("Time:"),space1)),fold_many1(terminated(complete::u32, space0), 
    Vec::new, |mut acc:Vec<u32>, item| {
        acc.push(item);
        acc
    }))(times_str)?;
    let (_,distances) = preceded(tuple((tag("Distance:"),space1)),fold_many1(terminated(complete::u32, space0), 
    Vec::new, |mut acc:Vec<u32>, item| {
        acc.push(item);
        acc
    }))(distance_str)?;
...
fn evaluate_race(race : &Race) -> u32 {
...
fn solve(input: &str) -> u32 {
    ...
    let ways_to_win:Vec<u32> = races.iter().map(|race| evaluate_race(race)).collect();
