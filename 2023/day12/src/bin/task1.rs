...
fn matches_pattern(line : &Vec<char>, pattern: &Vec<usize>) -> bool {
    ...
    while current_char < line.len() && current_pattern < pattern.len() {
        ...
        if c=='#' {
            counter = 0;
            if current_char+pattern[current_pattern] > line.len() { return false; }
            ...
            if counter == pattern[current_pattern] {
                current_char = current_char + pattern[current_pattern];
...
fn solve_line(line : &str) -> usize {
    ...
    let number_of_questions = springs.chars().filter(|&c| c=='?').collect::<Vec<char>>().len();
...
mod tests {
    ...
    fn patterns() {
        ...
        assert!(matches_pattern(&example.chars().collect::<Vec<char>>(),&pattern));
        println!("Example 2:");
        let ex2 = vec!['#', '.', '#', '.', '#', '#', '#'];
