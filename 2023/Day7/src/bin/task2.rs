...
fn hand_to_map(hand : &Hand) -> HashMap<u32,u32> {
    ...
    if map.contains_key(&0) {
        let num_jokers = map.remove(&0).expect("Joker");
        if num_jokers == 5 {
            //println!("Hand to Map: {:?}",hand);
            return map;
        }
        let mut highest_key = map.keys().next().unwrap();
        for (key,value) in map.iter() {
            if *value > map[highest_key] {
                highest_key=key;
            }
        }
        map.insert(*highest_key, map[highest_key]+num_jokers);
